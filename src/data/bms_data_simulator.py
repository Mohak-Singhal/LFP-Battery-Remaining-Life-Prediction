...
MIDC_TIME_S = np.array([0,10,20,30,40,50,60,70,90,110,130,150,170,200,230,260,300,340,380,420,460,500,540,580,620,660,700,760,820,880,940,1000,1060,1080], dtype=float)

MIDC_SPEED_KMH = np.array([0,10,20,30,35,30,25,0,15,30,40,50,45,35,25,10,0,20,35,45,55,60,55,45,35,20,0,20,35,50,60,40,20,0], dtype=float)


def build_midc_current_shape(time_s, speed_kmh):
...
def ocv_cell_from_soc(soc_pct):
    s = np.clip(soc_pct / 100.0, 0.0, 1.0)
    return 3.05 + 0.22 * s + 0.05 * np.tanh((s - 0.50) / 0.11) + 0.03 * (s - 0.50) ** 2


def simulate_cycle_profiles(rng, current_shape, dt_base, c_rate_target, dod_target, soc_start, capacity_ah, regen_eff):
...
def simulate_temperature_profile(i_profile, dt, ambient_c, r_internal_ohm, rng):
    c_th = 55000.0
    h_cool = 32.0
    ...
    return np.clip(temp, ambient_c - 2.0, 56.0)


def degradation_per_cycle(cycle_idx, temp_c, dod_pct, c_rate, base_deg_pct, battery_factor, rng):
    ...
    delta = base_deg_pct * battery_factor * temp_factor * dod_factor * c_rate_factor * aging_factor * stochastic
    return float(np.clip(delta, 0.0010, 0.0200))


def simulate_battery(battery_id, rng, current_shape, dt_base, max_cycles=6000):
    n_series = 110
    capacity_ah = 100.0
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        i_profile, dt, soc_trace, dod_actual = simulate_cycle_profiles(
            rng, current_shape, dt_base, c_rate_target, dod_target, soc_start, capacity_ah, regen_eff
        )

        r_internal = r0 * (1.0 + 0.012 * (100.0 - soh))
        temp_trace = simulate_temperature_profile(i_profile, dt, ambient_c, r_internal, rng)
        ...
        delta_soh = degradation_per_cycle(cycle, cell_temp, dod_actual, c_rate_cycle, base_deg_pct, battery_factor, rng)

        soh_next = max(soh - delta_soh, 80.0)
        ...
        effective_deg_rate = max(deg_rate_ema, 20.0 / 4500.0)
...
def generate_dataset(n_batteries=200, seed=42, max_cycles=6000):
    rng = np.random.default_rng(seed)
    current_shape, dt_base = build_midc_current_shape(MIDC_TIME_S, MIDC_SPEED_KMH)
    frames = []

    for bid in range(1, n_batteries + 1):
        frames.append(simulate_battery(bid, rng, current_shape, dt_base, max_cycles))
...
if __name__ == "__main__":
    ...
    output_path = output_dir / "synthetic_battery_rul.csv"
